...
void I2C_ApplicationEventCallback(I2C_Handle_t *pI2CHandle, uint8_t AppEv)
{
	...
	{
		//Master requests data, slave to send data
		if(commandCode == 0x51)
		{
	...
	{
		//Data is waiting for the salve to read. Slave has to read data
		commandCode = I2C_SlaveReceiveData(pI2CHandle->pI2Cx);
	}
	else if (AppEv == I2C_ERROR_AF)
	{
		//This happens only during slave Txing
		//Master has sent the NACK. Slave should understand that master doesn't
		//need more data.
		commandCode = 0xff;
		Count = 0;
	}
