 ...
 */
void SPI_PeriClckCtrl(SPI_RegDef_t *pSPIx, uint8_t EnOrDi)
{
	if(EnOrDi == ENABLE)
	{
		if(pSPIx == SPI1)
...
uint8_t SPI_SendDataIT(SPI_Handle_t *pSPIHandle,uint8_t *pTxBuffer, uint32_t Len)
{
	...
	//4. Data Transmission will be handled by the ISR code (TODO will implement later)
...
static void spi_txe_interrupt_handle(SPI_Handle_t *pSPIHandle)
{
	...
	if(! pSPIHandle->TxLen)
	{
		//TxLen is zero, so close SPI transmission and inform the application TX is over
...
static void spi_rxne_interrupt_handle(SPI_Handle_t *pSPIHandle)
{
	...
	if(! pSPIHandle->RxLen)
	{
		//RxLen is zero, so close SPI transmission and inform the application RX is over
